...
export async function deleteCategory(userId: string, category_name: string) {
	try {
		await sql`
            DELETE FROM UserCategories WHERE user_id = ${userId} AND category_name = ${category_name};
        `;
		await sql`
            UPDATE UserWords
            SET category_name = 'uncategorized'
            WHERE user_id = ${userId} AND category_name = ${category_name}
        `;
	} catch (error) {
		console.error(error);
	}
